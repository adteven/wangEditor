...
class UploadVideo {
    ...
    public uploadVideo(files: FileList | File[]): void {
        ...
        // 上传视频的最大体积，默认 1024M
        const uploadVideoMaxSize = config.uploadVideoMaxSize
        // 一次最多上传多少个视频
        ...
        arrForEach(files, file => {
            ...
            if (uploadVideoMaxSize < size) {
                // 上传图片过大
                errInfos.push(`【${name}】${t('大于')} ${uploadVideoMaxSize / 1024}M`)
                return
            }
        ...
        //     config.customAlert(t('一次最多上传') + uploadVideoMaxLength + t('个视频'), 'warning')
        //     return ;
        // }
